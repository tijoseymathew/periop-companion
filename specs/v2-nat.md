...
A second, smaller gap: the README and `docs/provenance.md` already claim OTel export ("NAT OTel traces record which agent, prompt, and model produced each artifact"). `pyproject.toml` installs `nvidia-nat[adk,eval,profiler]` — no `opentelemetry` extra, and no config file sets `general.telemetry.tracing`. Today the only real consumer of the intermediate-step stream is the profiler *during `nat eval`*. The OTel claim is true of the toolkit's capability, not of this repo's configuration.
